        ...
        <div class="card-premium border-0 shadow-sm">
            <div class="card-header bg-white border-0 pt-4 pb-0 px-4">
                <div class="d-flex align-items-center mb-3">
                    <div class="avatar bg-danger bg-opacity-10 text-danger rounded p-2 me-3">
                        <i class="bi bi-box-arrow-up fs-4"></i>
                    </div>
                    <div>
                        <h5 class="mb-0 font-weight-700">Tarik Tabungan</h5>
                        <p class="text-muted mb-0 small">Catat penarikan tabungan untuk banyak siswa sekaligus.</p>
                    </div>
                </div>
            </div>

            <div class="card-body p-4">
                <form action="{{ route('savings.store-bulk') }}" method="POST" id="bulkWithdrawForm">
                    @csrf
                        ...
                        <table class="table table-borderless align-middle" style="min-width: 800px;">
                            <thead class="bg-light text-muted">
                                <tr>
                                    ...
                                    <th width="25%">Nominal Tarik (Rp) <span class="text-danger">*</span></th>
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        <button type="button" class="btn btn-outline-danger border-dashed" id="btn-add-row">
                            <i class="bi bi-plus-circle me-1"></i> Tambah Baris
                        </button>
                        
                        <div class="text-end">
                            <h6 class="text-muted mb-1">Total Penarikan:</h6>
                            <h4 class="text-danger font-weight-700 mb-0" id="total-amount-display">Rp 0</h4>
                        </div>
                    </div>
                    ...
                    <div class="d-flex justify-content-end gap-2">
                        ...
                        <button type="submit" class="btn btn-danger" id="btn-submit">
                            <i class="bi bi-check-circle me-1"></i> Simpan Penarikan
                        </button>
                    </div>
                </form>
            </div>
    ...
    <tr class="transaction-row border-bottom border-light">
        ...
        <td>
            <div class="input-group">
                ...
                <input type="text" class="form-control border-start-0 amount-input text-danger" placeholder="0" oninput="formatRupiah(this); calculateTotal()">
                <input type="hidden" class="amount-hidden" name="students[{INDEX}][amount]">
            </div>
        </td>
        <td>
            <input type="text" class="form-control" name="students[{INDEX}][notes]" placeholder="Catatan penarikan...">
        </td>
        <td class="text-center">
            <button type="button" class="btn btn-sm btn-outline-secondary btn-remove-row" onclick="removeRow(this)" title="Hapus Baris">
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        document.getElementById('bulkWithdrawForm').addEventListener('submit', function(e) {
            ...
            // Confirm withdrawal
            if(!confirm('Apakah Anda yakin data penarikan ini sudah benar? Aksi ini akan mengurangi saldo siswa yang diisi nominalnya.')) {
                e.preventDefault();
                return false;
            }
